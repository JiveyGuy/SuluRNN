...
########################################################################################
# Model by Sherjil Ozair (Source: https://github.com/sherjilozair/char-rnn-tensorflow) #
########################################################################################
...
# Model which powers train and sample
class Model():
	########################################################################################
	# Model by Sherjil Ozair (Source: https://github.com/sherjilozair/char-rnn-tensorflow) #
	########################################################################################
...
def interactTrain(source, epoch, size, seqLen, batch, decay):
	...
	parser.add_argument('--rnn_size', type=int, default=size,
                    help='size of RNN hidden state')
	parser.add_argument('--num_layers', type=int, default=3,
                    help='number of layers in the RNN')
	# Optimization
	parser.add_argument('--seq_length', type=int, default=seqLen,
	...
	parser.add_argument('--learning_rate', type=float, default=0.002,
                    help='learning rate')
	parser.add_argument('--decay_rate', type=float, default=decay,
...
# BFC allocation to stop memory limit exception
with tf.Session(config = config) as s:
	# Training model
	interactTrain(source, epoch, size, seqLen, batch, decay)
	
# This is a very lazy way of implementing the code provided by Sherjil Ozair
# so that minimal code changes are needed. A better implementation would be to get rid of 
# save/load procedure for the model and make it a one time use. 
tf.reset_default_graph()

with tf.Session(config = config) as s:	
# Generating text of length n (chars) 295820 because the ratio of chars to seconds of audio is 42260.137931 and we want seven seconds. 
	generatedSound = interactSample(295820)

# Saving file with name containing model init info delimited by space \s
fileName = str(epoch)+"_"+str(size)+"_"+str(seqLen)+"_"+str(batch)+"_"+str(decay)+".txt"
output = open(fileName, "w")

#Printing because it lets me see generated text (reassuring)
print(generatedSound)

output.write(generatedSound)
